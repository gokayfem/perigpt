...
class PeriDynamicAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # ---- linearity trick: window projected disp, then subtract ---------
        ...
        # Use direct dot product instead of nn.Linear for (bd → 1) projections.
        # Avoids cuBLAS launch overhead for tiny matmuls.
        bond_act = F.gelu(bond_feats + pos_feat)           # (B, nh, T, δ, bd)
        bond_logits = (bond_act * self.bond_out.weight.squeeze(0)).sum(-1)  # dot product
        if self.bond_out.bias is not None:
            bond_logits = bond_logits + self.bond_out.bias.squeeze(0)

        dmg_act = F.gelu(damage_feats)
        damage = torch.sigmoid(
            (dmg_act * self.damage_out.weight.squeeze(0)).sum(-1)
            + self.damage_out.bias.squeeze(0)
        )
